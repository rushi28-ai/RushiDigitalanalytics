...
query = (
  ...
  .format("delta")
  .option("checkpointLocation", "dbfs:/FileStore/checkpoints/invoices")
  .outputMode("append")
...
# MAGIC %md
# MAGIC ###### Your ingestion code will not break but silently ignore the additional data
...
write_query = (
  source_df.writeStream
  .format("delta")
  .option("checkpointLocation", f"dbfs:/FileStore/checkpoints/invoices")
  .outputMode("append")
  .trigger(availableNow = True)
  .toTable("invoices")
)
...
# MAGIC %fs rm -r dbfs:/FileStore/checkpoints/invoices
