...
-- MAGIC %md
-- MAGIC - This is continuation of previous notebook
...
-- MAGIC students1_df = spark.createDataFrame([Row(**student) for student in students1['students']])
...
-- MAGIC %python
-- MAGIC #students1_df.write.insertInto("students")
-- MAGIC students1_df.createTempView("students1")
...
-- MAGIC students2_df = spark.createDataFrame([Row(**student) for student in students2['students']])
-- MAGIC display(students2_df)

-- COMMAND ----------

-- MAGIC %python
-- MAGIC students2_df.createTempView("students2")
